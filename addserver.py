...
"""
受け取った8byteのフィールドを2つに分け、4byte整数として解釈しレスポンスする。
TCP/IPの仕様によってネットワークバイトオーダー（ 
    = ビッグエンディアン；メモリの下位アドレスから上位アドレスに順に割り当て、
    上位アドレスから送る
）で値を受け取り、ホストバイトオーダーに変換し処理する。
送信前にネットワークバイトオーダーに再変換する。
"""
...
def main():
    ...
    print(f'received_msg: {received_msg}')
